...
/**
 * Deterministic JSON stringifier to ensure consistent key ordering.
 */
...
function calculateBlockHash(index, timestamp, data, previousHash, nonce) {
  ...
  const rawString = `${index}-${normalizedTimestamp}-${serializedData}-${previousHash}-${nonce}`;
...
async function verifyRecord(recordId, currentPayload) {
  ...
  const anchorBlock = blocks.find(
    (b) => b.data && typeof b.data === 'object' && b.data.type === 'RECORD_ANCHOR' && b.data.recordId === recordId
  );
...
async function getAuditTrail(patientDid) {
  ...
  return blocks
    .filter((b) => b.data && typeof b.data === 'object' && b.data.patientDid === patientDid)
    .map((b) => ({
      index: b.index,
      txId: b.hash,
